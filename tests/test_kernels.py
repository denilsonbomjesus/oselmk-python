...
from oselmk.utils.kernels import kernel_matrix, SUPPORTED_KERNELS
...
def test_omega_train_shape(ktype):
    ...
    omega = kernel_matrix(X_TRAIN, X_TRAIN, kernel=ktype, params=KERNEL_PARAMS[ktype])
...
def test_omega_test_shape(ktype):
    ...
    omega = kernel_matrix(X_TRAIN, X_TEST, kernel=ktype, params=KERNEL_PARAMS[ktype])
...
def test_rbf_diagonal_is_one():
    ...
    omega = kernel_matrix(X_TRAIN, X_TRAIN, kernel="rbf", params=1.0)
...
def test_rbf_invalid_sigma():
    ...
    with pytest.raises(ValueError, match="sigma > 0"):
        kernel_matrix(X_TRAIN, X_TRAIN, kernel="rbf", params=-1.0)


def test_rbf_larger_sigma_smoother():
    """Larger sigma -> values closer to 1 (wider Gaussian)."""
    omega_tight = kernel_matrix(X_TRAIN, X_TRAIN, kernel="rbf", params=0.01)
    omega_wide  = kernel_matrix(X_TRAIN, X_TRAIN, kernel="rbf", params=1000.0)
...
def test_linear_equivalence_with_dot_product():
    ...
    omega = kernel_matrix(X_TRAIN, X_TRAIN, kernel="linear", params=1.0)
...
def test_linear_cross_equivalence():
    ...
    omega = kernel_matrix(X_TRAIN, X_TEST, kernel="linear", params=1.0)
...
def test_polynomial_degree2_c0_equals_linear_squared():
    ...
    omega_poly = kernel_matrix(X_TRAIN, X_TRAIN, kernel="poly", params=[0.0, 2])
    omega_lin  = kernel_matrix(X_TRAIN, X_TRAIN, kernel="linear", params=1.0)
...
def test_polynomial_missing_params_raises():
    ...
    with pytest.raises(ValueError, match="params=\\[c, d\\]"):
        kernel_matrix(X_TRAIN, X_TRAIN, kernel="poly", params=2.0)
...
def test_wavelet_missing_params_raises():
    ...
    with pytest.raises(ValueError, match="params=\\[b, a, w0\\]"):
        kernel_matrix(X_TRAIN, X_TRAIN, kernel="wavelet", params=[1.0, 1.0])


def test_wavelet_symmetric():
    """Wavelet Omega_train must be symmetric."""
    omega = kernel_matrix(X_TRAIN, X_TRAIN, kernel="wavelet", params=[1.0, 1.0, 1.0])
    assert_allclose(omega, omega.T, atol=1e-12)
...
def test_unknown_kernel_raises():
    ...
    with pytest.raises(ValueError, match="Unknown kernel"):
        kernel_matrix(X_TRAIN, X_TRAIN, kernel="sigmoid", params=1.0)
...
def test_1d_input_is_accepted():
    ...
    omega = kernel_matrix(x, x, kernel="rbf", params=1.0)
